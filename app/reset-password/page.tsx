...
import { Shield, ArrowLeft, Mail, Loader2, Lock, Eye, EyeOff, CheckCircle2, Circle, CheckCircle, Activity } from 'lucide-react'
...
export default function ResetPasswordPage() {
  ...
  const [verifying, setVerifying] = useState(true) // ✅ NEW: Prevents showing email box early
  ...
  useEffect(() => {
    // 1. Check if we have a code in the URL immediately
    const hasCode = window.location.search.includes('code=')
    
    // 2. Listen for the recovery handshake
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === "PASSWORD_RECOVERY" || session) {
        setIsRecoverySession(true)
        setVerifying(false)
      } else if (!hasCode) {
        // Only stop verifying if there's no code in the URL
        setVerifying(false)
      }
    })

    // 3. Safety Timeout: If it takes too long to detect session, show request form
  ...
  // Strength Logic (Standard Unstra Protocol)
  ...
  const handleRequestLink = async (e: React.FormEvent) => {
    ...
    else setFeedback({ type: 'success', msg: "Recovery link dispatched. Check your inbox." })
  }

  const handleUpdatePassword = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!passwordStrength.isValid || !isMatch) return
    setLoading(true)
  ...
  // ✅ LOADING STATE: Prevents the "Email Box" from flickering
  if (verifying) return (
    <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center p-6 text-center">
      ...
      <p className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500">Establishing Recovery Session...</p>
    </div>
  )
  ...
  if (done) return (
    <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center p-6 text-center">
      ...
      <button onClick={() => router.push('/login')} className="mt-10 bg-white text-black font-black px-10 py-4 rounded-xl text-[12px] uppercase tracking-widest hover:bg-zinc-200 transition-all shadow-2xl">Enter Vault</button>
    </div>
  )
        ...
        <form onSubmit={isRecoverySession ? handleUpdatePassword : handleRequestLink} className="space-y-6">
          {!isRecoverySession ? (
            <div className="space-y-2 text-left">
              <label className="text-[12px] font-black uppercase tracking-[0.2em] italic ml-1">Primary Email</label>
              <input 
                className="w-full rounded-2xl px-5 py-4 bg-zinc-900 border border-white/20 focus:border-blue-500 outline-none transition-all font-bold text-sm" 
            ...
            <div className="space-y-6 text-left">
              <div className="space-y-2">
                <label className="text-[12px] font-black uppercase tracking-widest italic ml-1 text-blue-500">New Master Password</label>
              ...
              <div className="space-y-2">
                <label className="text-[12px] font-black uppercase tracking-widest italic ml-1">Confirm Protocol</label>
                <input 
          ...
          <button disabled={loading} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-[900] text-[12px] uppercase tracking-[0.4em] py-4.5 rounded-xl transition-all shadow-2xl active:scale-95">
            {loading ? <Loader2 className="animate-spin mx-auto" size={18} /> : (isRecoverySession ? 'CONFIRM NEW CREDENTIALS' : 'Dispatch Recovery Link')}
          </button>

          {feedback && (
            <p className={`p-4 rounded-xl text-[10px] font-black uppercase tracking-widest border text-center italic ${feedback.type === 'error' ? 'bg-red-500/10 text-red-500 border-red-500/20' : 'bg-blue-500/10 text-blue-400 border-blue-500/20'}`}>
              {feedback.msg}
            </p>
          )}
        </form>
